...
# VERSION=3 (2026-02-03): BUG-10 fix - Check transcript for *ing... pattern before sending webhook
...
log "transcript_path: ${TRANSCRIPT_PATH:-<not provided>}"
...
# Agent window names (not indices - windows are named by agentId)
AGENT_WINDOWS="sm dev architect architect-engineering qa"

normalize_agent_id() { case "$1" in architect-engineering) echo "architect";; *) echo "$1";; esac; }
...
find_window_for_agent() {
    ...
    if [[ "$agent_id" == "architect" ]]; then
        local eng_match=$(tmux list-windows -t "$session" -F '#{window_name}' 2>/dev/null | grep "^architect-engineering" | head -1)
        if [[ -n "$eng_match" ]]; then echo "$eng_match"; return 0; fi
    fi
...
if [[ -z "$AGENT_ID" ]]; then
    for agent in $AGENT_WINDOWS; do
        ...
        if [[ -n "$PANE_PID" ]] && ps -p "$PANE_PID" -o args= 2>/dev/null | grep -q "claude"; then
            WINDOW_NAME="$agent"
